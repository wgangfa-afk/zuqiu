...
The default idempotency key is a SHA-256 hash of canonical JSON. Every identity includes the evidence type, `fixture_id`, `provider`, `raw_payload_hash`, `mapping_version`, and normalized `observed_at_utc`. Team metrics additionally include side and metric name; availability includes side plus player reference/name; lineups include side, player reference/name, and lineup status. Fixture context includes its evidence-type marker. Thus two fixtures can never share an evidence record, and different observation times remain distinct time-series facts. An exact retry has the same key and every normalized persisted business field equal. Reusing a key with any different field is an `IdempotencyConflictError`: it fails closed, inserts nothing, and does not overwrite history.
