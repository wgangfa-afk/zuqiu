...
P0_TABLES = ("fixtures", "market_snapshots", "executions", "settlements", "bankroll_ledger", "analysis_decisions")
...
def startup_integrity_check(database: Database) -> IntegrityReport:
    errors: list[str] = []
    try:
        with database.connection() as connection:
            ...
            if connection.execute("SELECT COUNT(*) FROM schema_version").fetchone()[0] != 1:
                errors.append("schema version is not singular")
    ...
    if errors:
        ...
        database.record_audit("integrity_check_failed", payload_json=json.dumps({"errors": errors}))
    return IntegrityReport(ok=not errors, errors=tuple(errors))


def create_backup(database: Database, destination: Path, *, git_commit_sha: str | None = None) -> Path:
    ...
    with database.connection() as source, sqlite3.connect(database_file) as target:
    ...
    manifest = {
        ...
        "database_path": database_file.name, "schema_version": schema_version, "git_commit_sha": git_commit_sha,
        "db_size": database_file.stat().st_size, "sha256": _sha256(database_file), "row_counts": row_counts,
    ...
    database.record_audit("backup_created", payload_json=json.dumps({"backup_id": backup_id}))
