...
class Phase1AnalysisService:
    ...
    def analyze(
        ...
        *,
        ...
        for model in model_probabilities:
            if model.snapshot_id in probability_by_id:
                raise AnalysisInputError("duplicate model probability")
        ...
        for group in market_groups:
            if (
                ...
                or not group.market_key.strip()
                or group.market_key in keys
            ):
                raise MarketGroupError("invalid or duplicate market key")
            keys.add(group.market_key)
            if (
                type(group.exhaustive) is not bool
                or type(group.mutually_exclusive) is not bool
                or not group.exhaustive
                or not group.mutually_exclusive
            ):
                raise MarketGroupError(
                    "market group must be exhaustive and mutually exclusive booleans"
                )
            if len(group.snapshot_ids) not in (2, 3):
                raise MarketGroupError("market group must contain two or three selections")
            if len(set(group.snapshot_ids)) != len(group.snapshot_ids):
                raise MarketGroupError("duplicate snapshot id")
            if expected_ids.intersection(group.snapshot_ids):
                raise MarketGroupError("snapshot appears in multiple groups")
            ...
            if missing:
                raise AnalysisInputError("missing model probability")
            candidates.extend(self._price_group(group, snapshots, probability_by_id))

        if set(probability_by_id) != expected_ids:
            raise AnalysisInputError("extra model probability")

        def quality(item: PricedSelection) -> int:
            if item.validation_status == "VALID":
                return 0
            if item.validation_status == "STALE":
                return 1
            return 2
        ...
        ranked = tuple(
            sorted(
                candidates,
                key=lambda item: (
                    item.risk_adjusted_ev is None,
                    -numeric(item.risk_adjusted_ev),
                    -numeric(item.raw_ev),
                    quality(item),
        ...
        hard = {
            ReasonCode.UNSUPPORTED_SETTLEMENT_MODEL,
            ReasonCode.STALE_MARKET_DATA,
            ReasonCode.UNVERIFIED_MARKET_DATA,
        }
        selectable = next(
            (
                item
                for item in ranked
                if item.rating in {"B+", "B"}
                and item.action is DecisionAction.WATCH
                and item.risk_adjusted_ev is not None
                and not hard.intersection(item.reason_codes)
            ),
            None,
        )
    ...
    def _validate_group(self, group: MarketGroup, snapshots: tuple[MarketSnapshotRecord, ...]) -> None:
        if any(item.fixture_id != group.fixture_id for item in snapshots):
            raise MarketGroupError("mixed fixture group")
        if (
            len({item.market_type for item in snapshots}) != 1
            or len({item.settlement_type for item in snapshots}) != 1
        ):
            raise MarketGroupError("mixed market group")
        if len({item.line for item in snapshots}) != 1:
            raise MarketGroupError("mixed line group")
        if len({item.observed_at_utc for item in snapshots}) != 1:
            raise MarketGroupError("mixed observation time group")
        if len({item.selection for item in snapshots}) != len(snapshots):
            raise MarketGroupError("duplicate selection")
        ...
        for snapshot in snapshots:
            valid_odds(snapshot.decimal_odds)

    def _price_group(
        self,
        group: MarketGroup,
        snapshots: tuple[MarketSnapshotRecord, ...],
        models: dict[str, ModelProbability],
    ) -> tuple[PricedSelection, ...]:
        ...
        if abs(sum(models[item.id].probability for item in snapshots) - 1) > 1e-9:
            raise InvalidProbabilityError("group model probabilities must sum to one")
        output = []
        for snapshot, implied_probability, devig_probability in zip(snapshots, implied, devig_probabilities):
            ...
            if snapshot.validation_status == "STALE":
                reasons.append(ReasonCode.STALE_MARKET_DATA)
            elif snapshot.validation_status != "VALID":
                reasons.append(ReasonCode.UNVERIFIED_MARKET_DATA)
            if raw_ev > 0:
                reasons.append(ReasonCode.POSITIVE_RAW_EV)
            if adjusted > 0:
                reasons.append(ReasonCode.POSITIVE_RISK_ADJUSTED_EV)
            if adjusted <= 0:
                reasons.append(ReasonCode.NO_POSITIVE_EDGE)
                if raw_ev > 0:
            ...
            if action is DecisionAction.WATCH:
                reasons.extend(
                    (ReasonCode.PHASE1_RATING_CAP, ReasonCode.PHASE1_EXECUTION_DISABLED)
                )
            elif rating == "C":
                reasons.extend(
                    (ReasonCode.BELOW_WATCH_THRESHOLD, ReasonCode.PHASE1_EXECUTION_DISABLED)
                )
